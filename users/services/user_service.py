...
class UserService:
    ...
    def soft_delete_account(user):
        ...
        if orphaned_projects:
            raise AccountIsLastProjectManagerError(
                orphaned_projects
            )
        ...
        unassigned_bug_count = active_bugs.update(
            assigned_to=None,
        )

        project_memberships_removed, _ = (
            ProjectRole.objects
            .filter(user=locked_user)
            .delete()
        )

        workspace_memberships_removed, _ = (
        ...
        rejected_invitations = Invitation.objects.filter(
            Q(receiver=locked_user)
            | Q(
                receiver_email__iexact=locked_user.email
            ),
            status="PENDING",
        ).update(
            status="REJECTED",
            receiver=locked_user,
        )
        ...
        create_activity_log(
            ...
            changes={
                ...
                "reason": (
                    "Account soft-deleted by its owner."
                ),
                "soft_delete": True,
                "unfinished_tasks_unassigned": (
                    unfinished_task_count
                ),
                "active_bugs_unassigned": (
                    unassigned_bug_count
                ),
                "project_memberships_removed": (
                    project_memberships_removed
                ),
                "workspace_memberships_removed": (
                    workspace_memberships_removed
                ),
                "pending_invitations_rejected": (
